...
public class PoliceDispatchSimulation {
	...
	public void run() {
		// FIXME
		while (eventQueue.size() > 0 || triageQueue.size() > 0) {
			Incident first = eventQueue.peek().value;
			int time = -1;
			if (eventQueue.size() > 0) {
				time = first.getTimePriority();
			}
			while (eventQueue.size() > 0 && first.getTimePriority() == time && eventQueue.size() != 0) {
				if (first.dispatched()) {
					//resolve incident
					logResolution(time, first);
					markUnitAvailable(first.getDispatchedUnit());
					eventQueue.poll();
					}
				else {
					logReport(time, first);
					triageQueue.add(eventQueue.poll(), first.getTriagePriority());
				}
				first = eventQueue.peek().value;
			}
			Incident firstDispatch = triageQueue.peek().value;
			while (triageQueue.size() > 0 && nextAvailableUnit() != -1) {
				firstDispatch.dispatch(time, nextAvailableUnit());
				logDispatch(time, firstDispatch);
				markUnitUnavailable(nextAvailableUnit());
				addToIncidentQueue(firstDispatch);
				triageQueue.poll();
				firstDispatch = triageQueue.peek().value;
			}
		}
	}
